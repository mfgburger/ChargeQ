...
def return_list_of_random_cars(number_of_cars):

    # Generate list of random Car objects using random_car_list() from CarList:
    list_of_cars = random_car_list(number_of_cars)
    
    # Compile a list of dicts containing each cars properties:
    list_of_random_cars = []
    for car in list_of_cars:
        properties = car.return_values()
        list_of_random_cars.append(properties)
        
    return list_of_random_cars
    
 # Only for testing/debugging:
if __name__ == "__main__":
    cars = return_list_of_random_cars(2)
    print(cars)
